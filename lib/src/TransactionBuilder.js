...
import {ChainTypes, hash, ops, PublicKey, Signature} from "gxbjs/es/index";

let expire_in_secs = 15;
...
class TransactionBuilder {
    ...
    finalize() {
        return new Promise((resolve) => {
            ...
            resolve(this.rpc.query("get_objects", [["2.1.0"]]).then((r) => {
    ...
    update_head_block() {
        return Promise.all([
            this.rpc.query("get_objects", [["2.0.0"]]),
            this.rpc.query("get_objects", [["2.1.0"]])
        ]).then(function (res) {
            let [g, r] = res;
            head_block_time_string = r[0].time;
    ...
    set_required_fees(asset_id) {
        ...
        var promises = [
            this.rpc.query("get_required_fees", [operations, asset_id])
        ];
        ...
        return Promise.all(promises).then((results) => {
            ...
            let dynamicPromise = (asset_id !== "1.3.1" && asset) ? this.rpc.query("get_objects", [[asset.dynamic_asset_data_id]]) : new Promise(function (resolve, reject) {
                resolve();
            });
            ...
            if (!this.signProvider) {
                ...
                for (var i = 0; 0 < end ? i < end : i > end; 0 < end ? i++ : i++) {
                    ...
                    var sig = Signature.signBuffer(
                        Buffer.concat([Buffer.from(this.chain_id, "hex"), this.tr_buffer]),
                        private_key,
                        public_key
                    );
                ...
                try {
                    this.signatures = await this.signProvider(this, this.chain_id);
                } catch (err) {
                    reject(err);
                    return;
                }
...
function _broadcast() {
    return new Promise(async (resolve, reject) => {
        ...
        resolve(this.rpc.broadcast(tr_object).catch(error => {
