...
class GXClient {
    ...
    constructor(private_key, account_id_or_name, witness = "wss://node1.gxb.io", signProvider = null) {
    ...
    _connect() {
        if (this.connected) {
            ...
            return this.connectingProcess;
        } else {
            return this.createNewConnect();
        }
    }

    createNewConnect() {
        this.isConnecting = true;
        console.log("always close the instance before we create new");
        return this.connectingProcess = Apis.close().then(() => {
            return Apis.instance(this.witness, true, undefined, {}, this._onClose.bind(this)).init_promise.then(() => {
                this.connectingProcess = null;
                this.isConnecting = false;
                this.connected = true;
                if (!!this.account_id_or_name && !this.account_id) {
                    return this.getAccount(this.account).then(account => {
     ...
     * @returns {Promise<any>}
     */
    transfer(to, memo, amount_asset, broadcast = false) {
        ...
        return new Promise((resolve, reject) => {
            if (amount_asset.indexOf(" ") == -1) {
                ...
                resolve(this._connect().then(() => {
                    return Promise.all([
                        Apis.instance().db_api().exec("get_objects", [[this.account_id]]),
                        this.getAccount(to),
                        this.getAsset(asset)
                    ]).then(async results => {
                        let memo_object;
     ...
     * @returns {Promise<any>}
     */
    callContract(contract_name, method_name, params, amount_asset, broadcast = false) {
        return this._connect().then(() => {
            if (amount_asset) {
                if (amount_asset.indexOf(" ") == -1) {
