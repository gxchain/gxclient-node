...
import {Aes, key, object_id_type, PrivateKey,PublicKey, string_to_name, TransactionBuilder, TransactionHelper} from "gxbjs";
...
class GXClient {
    ...
    vote(account_ids, overwrite = false, fee_asset_id, broadcast = false) {
        return new Promise((resolve) => {
            resolve(this._connect().then(() => {
                let tr = new TransactionBuilder();
                return this.getObject(this.account_id).then(acc => {
                    ...
                    return Promise.all(promises).then(results => {
                        ...
                        new_options.num_committee = num_committee;
                        new_options.num_witness = num_witness;
